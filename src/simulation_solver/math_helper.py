...
def compute_ke_quadratic(triangle_vertices, a_11, a_22):
    ...
    for i in range(3):
        for j in range(3):
            ke[i, j] = a_11 * (vertices[(i+1) % 3, 1] - vertices[(i+2) % 3, 1]) * (vertices[(j+1) % 3, 1] - vertices[(j+2) % 3, 1]) + \
                       a_22 * (vertices[(i+2) % 3, 0] - vertices[(i+1) % 3, 0]) * (vertices[(j+2) % 3, 0] - vertices[(j+1) % 3, 0])
            ke[i+3, j+3] = ke[i, j]
